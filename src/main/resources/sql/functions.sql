...
BEGIN
    SELECT user_login, user_row_version
        ...
        FROM users
        WHERE user_login = var_user.user_login AND user_pass = var_user.user_pass;
    IF temp_login IS NULL THEN RAISE EXCEPTION 'login error; %', var_user_login; END IF;
    UPDATE users
        SET user_ip = var_user.user_ip, user_row_version = user_row_version + 1
        WHERE user_login = var_user.user_login AND temp_rv = user_row_version;
    IF FOUND THEN
        return QUERY SELECT user_login, user_pass, user_ip FROM users WHERE users.user_login = var_user.user_login;
    ELSE
        RAISE EXCEPTION 'login failure; %', var_user.user_ip;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    SELECT users.user_login
        INTO temp_login
        FROM users
        WHERE users.user_ip = var_user.user_ip;
    IF temp_login IS NULL THEN RAISE EXCEPTION 'unregistered user with ip %', var_user.user_ip; END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF var_contact IS NULL THEN
        DELETE FROM contacts WHERE contact_name = var_contact_name;
        RETURN;
    END IF;
    ...
    IF temp_contact_name IS NULL THEN
        INSERT INTO contacts (contact_name, contact_email, contact_telegram, contact_num, contact_skype, group_id)
            values(var_contact.contact_name, var_contact.contact_email, var_contact.contact_telegram, var_contact.contact_num, var_contact.contact_skype, (SELECT group_id FROM groups WHERE group_name = var_contact.group_name))
    ...
    ELSE
        UPDATE contacts
            SET contact_email = var_contact.contact_email, contact_telegram = var_contact.contact_telegram, contact_num = var_contact.contact_num, contact_skype = var_contact.contact_skype, group_id = (SELECT group_id FROM groups WHERE group_name = var_contact.group_name)
            WHERE contact_name = var_contact.contact_name;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FOR dataReturned IN
        ...
        JOIN users
            ON users.user_id = contact_user.user_id AND users.user_login = var_user.user_login
        JOIN groups
...
BEGIN
    PERFORM check_access(var_user);
    IF var_group IS NULL THEN
        DELETE FROM groups WHERE group_name = var_group_name;
        RETURN;
    END IF;
    SELECT group_name INTO temp_group_name FROM groups WHERE group_name = var_group.group_name;
    ...
    ELSE
        UPDATE groups
            SET group_color = var_group.group_color
            WHERE group_name = var_group.group_name;
    END IF;
END;
$$ LANGUAGE plpgsql;
